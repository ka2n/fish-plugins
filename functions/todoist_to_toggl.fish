function todoist_to_toggl
  ...
  todoist --csv show $item_id | read -z item
  echo $item | grep 'Project' | cut -d',' -f2 | cut -d':' -f1 | sed 's/^#//' | read project_name
  echo $item | grep 'Content' | cut -d',' -f2- |  sed s/\"//g | read task_content
